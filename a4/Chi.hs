...
testSubstitution subst =
      ...
      (\e' ->
        ...
        (not (Set.null fs) && not (isVar e) ==>
         forAll (var fs) $ \x ->
           counterexample' x e e'
             (\x ->
               "However, eâ€³ = eâ€² (which should not hold, because " ++
               x ++ " is free in eâ€² and e is not a variable).") $
           subst x e e' /= e')
          .&&.
        (not (Set.null fbs) ==>
         forAll (var fbs) $ \x ->
           counterexample' x e e'
             (\x ->
               "However, bound eâ€³ â‰  bound e âˆª bound eâ€² (the two " ++
               "sides should be equal, because " ++ x ++ " is free " ++
               "in eâ€² and bound in e).") $
           bound (subst x e e')
             ==
  ...
  def s e =
    nest 2 $ case lines (pretty e) of
      [e] -> h <+> text e
      es  -> vcat (h : map (nest (length s + 3) . text) es)
    where
    h = text s <+> text "="
